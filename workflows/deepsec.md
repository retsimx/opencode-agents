---
description: Drive the `deepsec` skill end-to-end. Installs `.deepsec/`, calibrates cost, runs the right scan/process/triage/revalidate/export pass, gates PRs with `process --diff`, writes custom matchers, and routes findings to follow-up specialists.
---

# MANDATORY RULES: VIOLATION IS FORBIDDEN

- **NEVER skip steps.** Execute from Step 1 in order.
- **Do NOT modify product source code in this workflow.** Findings that need code edits hand off to `debug` / `backend` / `frontend` / `mobile` / `tf-infra` / `db` in Step 5.
- **Read the skill before acting.** Step 1 mandates loading `.agents/skills/deepsec/SKILL.md` and only the resource files needed for the resolved intent.
...
Read `.agents/skills/deepsec/SKILL.md` in full. Do **not** preload all `resources/*.md`. Load only what the resolved intent in Step 2 requires:
...
| Mobile / native client | `mobile` |
| IaC / cloud / network | `tf-infra` |
| Database / data model | `db` |
| CI / workflow / supply chain | `dev-workflow` (or this workflow's PR-review pattern for the deepsec gate itself) |
| Documentation drift surfaced by the run | `docs` |
| New entry-point gap (a cluster of `other-*` slugs or a framework deepsec does not glob) | re-enter Step 4D (`matchers`) |

**Ambiguity → `debug` first.** Route to `debug` whenever the layer is not obvious from the artifact: shared / isomorphic / utility code, an `other-*` slug, a fix that would touch multiple layers, `revalidation.verdict === "uncertain"`, or `BUG` / `HIGH_BUG` non-security correctness without an obvious owner. The hop is **triage, not fix**: pin the exact file:line and re-route to the correct specialist with a layer-tagged finding, or fix inline only when the change is a single isolated line and the diagnosis is confident. The deepsec run summary must record the second-hop owner so the user sees who finally took the work.
...
- Do NOT modify `.agents/` files unless the user is editing the agent source repo itself.
