---
description: Documentation drift detection and sync via `docs`. Verify mode finds broken refs in docs/**/*.md, sync mode proposes patches for docs affected by a git diff.
---
...
- **Follow the host-LLM contract** in `.agents/skills/docs/SKILL.md`: the CLI emits structured data; this workflow performs natural-language synthesis, severity grouping, and patch drafting on top of the JSON output.

---

> **Vendor note:** This workflow executes inline (no subagent spawning). Load the `docs` skill via the `skill` tool and follow its instructions for verify or sync mode.
...
```
Run `docs-verify` (drift check) or `docs-sync` (propose patches for a git diff)?
```
...
1. Load the `docs` skill via the `skill` tool to access verify and sync capabilities.
...
3. If `docs` skill is not available, print an install hint and exit. Do NOT silently substitute manual greps.
...
```bash
# Narrow scope
docs-verify "docs/**/*.md" --json
docs-verify cli/README.md --json
...
docs-verify --no-urls --json

# Block until lychee URL check finishes (CI-style)
docs-verify --urls-sync --json
```
...
# Explicit range
docs-sync HEAD~5..HEAD --json
docs-sync main..feature-branch --json
```
...
| Situation | Recovery |
|-----------|----------|
| `docs` skill not available | Print install hint; exit. Do not fall back to manual grep. |
| `lychee` missing | Print install hint (`brew install lychee`); continue with core check only. |
| `doc-refs.json` stale in sync | Run `docs-verify --json` first, then re-run sync. |
...
- Skill spec: `.agents/skills/docs/SKILL.md`
- Design doc: `docs/plans/designs/008-docs.md`
