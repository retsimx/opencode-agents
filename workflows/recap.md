---
description: Daily or period recap via `recap`. Resolves a date or window from natural language, invokes `recap --json`, delegates theme analysis and Markdown formatting to the skill, and reports a TL;DR plus saved path.
---
...
- **Follow the host-LLM contract** in `.agents/skills/recap/SKILL.md`: theme analysis, grouping rules, and Markdown output format are owned by the skill. This workflow only resolves intent, runs the CLI, and reports.
- **Never auto-translate technical terms** in the saved recap (project names, tool names, CLI flags).

---

> **Vendor note:** This workflow executes inline (no subagent spawning). Load the `recap` skill via the `skill` tool and follow its instructions.
...
1. Load the `recap` skill via the `skill` tool.
2. If the skill is not available:
   - Fall back to the Claude-history-only `jq` path documented in `recap` SKILL.md §2.
...
# Specific date
recap --date 2026-05-11 --json

# Specific date with explicit tool filter
recap --date 2026-05-11 --tool claude,codex --json
```
...
```bash
# Past N days
recap --window 7d --json
recap --window 30d --json

# Period with explicit tool filter
recap --window 7d --tool codex --json
```
...
Hand the JSON output to `recap` SKILL.md. The skill owns:
...
- Fallback (jq) path was used → "Coverage: Claude history only (recap skill not available)"
...
| Situation | Recovery |
|-----------|----------|
| `recap` skill not available | Use the `jq` fallback from SKILL.md §2; note limited coverage. |
| No `~/.claude/history.jsonl` and no `recap` skill | Exit with missing-source message; do not invent data. |
...
- Skill spec: `.agents/skills/recap/SKILL.md`
- Output directory: `.agents/results/recap/`
- Claude fallback history: `~/.claude/history.jsonl`
- Related: `retro` (Git commit retrospective — different scope)
