...
3. If still empty: run `web-fetch <url>` on candidate URLs

### Native search blocked (402/403)
**Symptom**: Target site returns access denied
**Recovery**:
1. Run `web-fetch <url>`; this auto-escalates api → probe → impersonate → browser
...
**Symptom**: `"Chrome/Chromium not found ... or set CHROME_PATH."`
**Recovery**:
1. Load the `search` skill to diagnose search state.
2. Install Chrome / Edge / Brave / Chromium, or set `CHROME_PATH`.
...
**Recovery**:
1. Skip native search, go directly to `web-fetch <url>`
...
**Recovery**: Use runtime-agnostic tool names (generic read/search/execute semantics).
