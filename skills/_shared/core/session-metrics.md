...
- `false_negative`, `severity_mismatch`: Recorded retroactively via `retro` or next session discovery
...
| Threshold | Action |
|-----------|--------|
| EA >= 30 | **TUNING SUGGESTED**: `retro` flags QA patterns for review |
...
- **Precise tokens**: Available only via `stats` post-hoc (parses CLI logs when supported)
