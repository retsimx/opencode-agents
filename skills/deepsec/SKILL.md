...
- Pure reasoning about a finding's fix in product code → use `debug` once deepsec has produced the finding.
...
- Does not commit, push, or open PRs unless the user explicitly authorizes a separate commit step (route via `scm`).
