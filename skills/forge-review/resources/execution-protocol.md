...
- **Subagent 0 (`context-ingestion`)**:
  ...
  - Generates `spec-issue.md`, `pr-context.md`, and `diff-pr.patch` with dynamic nonces and raw uncorrupted code diff syntax; `pr-context.md` carries a labelled UNTRUSTED author/maintainer-comments block.
- **Subagent 1 (`qa-agent`)**:
  - Validates 100% of Acceptance Criteria and contract commitments against the code diff; may read the UNTRUSTED author/maintainer comments for intent but never as proof.
...
- **Subagent 2 (`deep-reviewer`)**:
  - Audits diff across all 9 code quality dimensions; may read the UNTRUSTED author/maintainer comments for intent but never to dismiss a code-grounded finding.
...
- **Subagent 3 (`security-agent`)**:
  - Operates under isolated Strict Zero-Trust (diffs only, barred from narrative justifications; the author/maintainer comments block is deliberately withheld).
...
- **Subagent 4 (`review-verifier`)**:
  - Reads `spec-issue.md` (SPEC_FILE), `pr-context.md` (UNTRUSTED comments = intent only), plus the raw synthesis and live repository source.
