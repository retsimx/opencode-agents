...
- [ ] Every deferred external integration is wrapped in `// TODO(deferred): <vendor or feature>`
- [ ] Every mock data block has `// MVP: replace with <real-source>` on top
- [ ] Every save/persist UI has a working storage adapter (localStorage, IndexedDB, or API). Empty `onClick={() => console.log("save")}` does NOT count; wire actual persistence, or mark explicitly: `// TODO(deferred): persist <state> to <storage>`
