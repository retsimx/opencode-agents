...
### When NOT to use
- Git commit-based code change retrospective -> use `retro`
- Real-time agent monitoring -> use `dashboard`
- Productivity metrics -> use `stats`

### Expected inputs
- Date, relative date, time window, or tool filter
- Conversation history available through `recap --json` or fallback sources
...
| Collect history | `CALL_TOOL` | `recap --json` or `jq` fallback |
...
| `PROCESS` | `recap`, `jq`, date commands |
...
recap --date 2026-04-10 --json

recap --tool claude,gemini --json
```
...
9. **No em dashes**: Use commas, periods, or parentheses instead of `—` (em dash).

## References

- Recap CLI: `recap --json`
