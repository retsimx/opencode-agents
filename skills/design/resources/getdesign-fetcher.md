...
**design intentionally rejects that framing.** We treat every
...
1. Upstream templates are 100% English and assume latin typography.
   design must honor SKILL.md Rule #3 (CJK fonts for ko/ja/zh
...
design always fetches the latest published `getdesign` release
...
1. Upstream ships new vendor templates and hash-verified manifest
   updates on its own cadence; pinning would leave design stale.
...
3. design treats templates as seeds, not finals. Minor upstream
...
GETDESIGN_DISABLE_TELEMETRY=1 bunx getdesign@latest add <brand> \
  --out "${TMPDIR:-/tmp}/design-seed-<brand>-$$.md" \
  --force
```
...
```bash
rm -f "${TMPDIR:-/tmp}/design-seed-<brand>-$$.md"
```
...
ACTUAL=$(shasum -a 256 "${TMPDIR:-/tmp}/design-seed-<brand>-$$.md" | awk '{print $1}')
...
- **Section 7 (Do's and Don'ts)**: merge with `anti-patterns.md`;
  ...
  that violate design anti-patterns.
