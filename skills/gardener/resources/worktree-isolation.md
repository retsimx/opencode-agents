# Gardener — Worktree Isolation (FATAL)
...
1. Every subagent prompt MUST receive `WORKTREE` (absolute path) when a worktree exists for the iteration.
...
6. Nested subagents spawned by any gardener subagent MUST receive the same `WORKTREE` path and obey these rules.
...
- `git branch -D <gardener-branch>` (local branch only, never `main`)
- Reading `.agents/results/gardener-state.md`
...
## Explicitly allowed in MAIN_REPO (state/metadata only)

- `edit` / `write` on `.agents/results/gardener-state.md` — SHIP appends new rows, INIT updates Status fields
