...
## Step -1: Clarify / Amplify Prompt (agent-side, before `image-generate`)
...
If the user asks to generate/edit an image AND a host-attached image is visible to the agent (e.g. `[Image: source: <path>]` in a Claude Code system message, Antigravity workspace upload, or explicit user-provided path), the agent MUST pass it via `--reference <path>`. Do not fall back to describing the image in prose. Do not ask the user to re-type the path. If `image-generate --help` shows no `--reference` flag, instruct the user to run `update-tooling` and retry; do not silently degrade.

### Host-Specific Reference Paths

Agents invoking `image-generate --reference` should surface the following host-specific locations to the user:
...
3. If estimate ≥ `cost_guardrail.estimate_threshold_usd` and not `--yes`/`FORCE_YES=1`:
...
1. For each run, print a one-line status to stderr:
   - `[image] <vendor> ok (Xs) -> <file>`
   - `[image] <vendor> failed (<kind>): <reason>`
...
| Situation | Action |
|-----------|--------|
| No vendors authenticated | Exit 5, print authentication instructions |
| Specific vendor unhealthy | Exit 5 with the vendor's setup guide (URL + env var + steps) |
