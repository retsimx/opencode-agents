...
3. **ACT**: Invoke `image-generate` with selected vendor(s), prompt, references, and options.
...
| Call generation CLI/API | `CALL_TOOL` | `image-generate` |
...
```bash
image-generate "<prompt>" --vendor auto --size auto --quality auto --format json
```

With reference images:
```bash
image-generate --reference "<absolute-path>" --vendor codex "<prompt>"
```
...
3. **Cost guardrail**: confirm before executing runs whose estimated cost is ≥ `$0.20` (configurable). `--yes` / `FORCE_YES=1` bypass. Default vendor `pollinations` (flux/zimage) is free, so auto-triggering on keywords is safe.
...
8. **Exit codes align with `web-fetch`** (0, 1, 2=safety, 3=not-found, 4=invalid-input, 5=auth-required, 6=timeout).

### Clarification Protocol

Before invoking `image-generate`, the calling agent runs this checklist against the user's request. **If any answer is "no / unknown", clarify with the user first.**
...
This skill follows a CLI-first concept: whenever a vendor's native CLI can drive generation (and return raw bytes), the subprocess path is preferred over direct API keys. Direct API is only used as a fallback for vendors whose CLI can't yet emit raw image bytes.
...
```
/image a red apple on white background
/image --vendor all --size 1536x1024 jeju coastline at sunset
/image -n 3 --quality high --out ./hero "minimalist dashboard hero illustration"
```

#### Shell CLI

```
image-generate "<prompt>" [--vendor auto|codex|pollinations|gemini|all] [-n 1..5] \
...
```
image-generate -r ~/Downloads/otter.jpeg "same otter in dramatic lighting"
image-generate -r a.png -r b.png "blend these two styles"
```
...
**Required action**: invoke `image-generate --reference <absolute-path> --vendor <codex|gemini> "<prompt>"`. If the user didn't specify a vendor, default to `codex` (CLI-first, widest availability). Do NOT:

- Fall back to prose description ("I'll describe the otter's appearance...").
- Ask the user to re-type or re-attach the path.
- Claim the CLI doesn't support references without first running `image-generate --help` to verify.

**If the local CLI is outdated** (`--reference` is missing from `--help`): tell the user to run `update-tooling` once, then retry. Do not silently degrade to prose.
...
Other skills call `image-generate --format json` and parse the JSON manifest from stdout.
