...
Standalone:
```
/observability "set up OTel stack on Kubernetes"
/observability --migrate "move from Fluentd to Fluent Bit"
/observability --investigate "5xx spike in ap-northeast-2"
/observability --alert "configure SLO burn-rate alert for checkout API"
/observability --trace "W3C propagator across AWS + GCP boundary"
/observability --tune "UDP statsd MTU throughput limit"
/observability --route "multi-tenant log isolation with data residency"
```
...
| `frontend` | `layers/L7-application/web-rum.md` INP/LCP/CLS checklist cross-referenced in `frontend.md` ruleset | ⏳ pending (v1.1) |
| `mobile` | `layers/L7-application/mobile-rum.md` offline-queuing pattern cross-referenced in `mobile.md` ruleset | ⏳ pending (v1.1) |
| `db` | `signals/traces.md` DB patterns (N+1, connection pool) cross-referenced in `database.md` ruleset | ⏳ pending (v1.1) |
