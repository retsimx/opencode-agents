...
This file is the coverage map for the `observability` skill. It answers one question per cell:
...
**Normative base:** all taxonomy, semconv stability tiers, and OSI boundary decisions are declared in `.agents/skills/observability/resources/standards.md`. Read that file before using this matrix.
...
- **C1. L3 × {SLO, release}**: N/A. SLO and release are L7 constructs (OpenSLO YAML, canary markers); L3 IP flow feeds infra health, not application error budgets. An L3 event (BGP leak, PMTUD black hole) that causes an SLO burn surfaces as an L7 error spike first; investigate via `.agents/skills/observability/resources/incident-forensics.md` 6-dim localization, not via L3 SLO policy. See `boundaries/slo.md`, `layers/L3-network.md`.
...
- **C4. {L3, L4} × traces (PARTIAL)**: W3C Trace Context lives in HTTP/gRPC headers, not IP/TCP. The PARTIAL reflects a correlation technique (log socket 5-tuple alongside trace ID) rather than native propagation. Native propagation begins at mesh or L7. See `.agents/skills/observability/resources/standards.md` §W3C Trace Context.
...
All files referenced in this matrix belong to the `observability` skill tree. Files not yet written (Phase 1b/1c per the design document rollout plan) are forward references.
...
| Resources | `.agents/skills/observability/resources/standards.md`, `.agents/skills/observability/resources/incident-forensics.md`, `.agents/skills/observability/resources/observability-as-code.md`, `.agents/skills/observability/resources/meta-observability.md` |

---

## 8. Review and Maintenance

- **Review cadence**: quarterly, aligned with `.agents/skills/observability/resources/standards.md` version update cadence.
