...
Cross-skill entry point: `debug` invokes this playbook on failure, pulling traces and logs by `request_id`. See the design document Integration table.
...
Release markers must be present as structured log events or metric annotations carrying `service.version`, `deployment.environment`, and the deployment SHA (propagated via `scm`). Without release markers, this step is blind; see anti-patterns below.
