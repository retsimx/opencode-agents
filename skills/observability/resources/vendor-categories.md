...
**Delegation target:** For Grafana stack setup, invoke `search --docs "Grafana LGTM+ OTel Collector setup"`. For Elastic, use Elastic documentation directly. No single vendor-owned skill covers the full OSS stack; delegate to `search` unless a specific component skill is installed.
...
**Delegation target:** `Datadog Labs dd-apm`, `getsentry/sentry-sdk-setup`. For New Relic / Dynatrace: `search --docs "{vendor} OTel integration setup"`.
...
**Delegation target:** `search --docs "Parca OTel profiling setup"` or `search --docs "Grafana Pyroscope integration"`.
...
**Delegation target:** `search --docs "{vendor} SIEM log pipeline setup"`. For audit-specific requirements see `signals/audit.md` in this skill.
...
**Delegation target:** `search --docs "OpenCost Prometheus integration"`. For cost signal context see `signals/cost.md` in this skill.
...
**Delegation target:** `search --docs "OpenFeature SDK {runtime} setup"` or `search --docs "Flagger canary analysis Prometheus"`. For release boundary observability see `boundaries/release.md` in this skill.
...
**Delegation target:** `search --docs "Fluent Bit Kubernetes DaemonSet setup"` or `search --docs "OTel Collector configuration {backend}"`. For collector topology guidance see `transport/collector-topology.md` in this skill.
...
**Delegation target:** `search --docs "Thanos sidecar Prometheus setup"` or `search --docs "Grafana Mimir distributed mode"`.
...
**Delegation target:** `getsentry/sentry-sdk-setup` for Sentry. For others: `search --docs "{vendor} iOS/Android crash reporting setup"`. For crash analytics context see `layers/L7-application/crash-analytics.md` in this skill.
...
```
search --docs "{vendor} observability setup"
search --docs "{vendor} OTel integration {runtime}"
search --docs "{vendor} {signal} configuration"
```

Examples:
- `search --docs "Grafana Mimir Prometheus remote_write setup"`
- `search --docs "OpenCost Kubernetes namespace cost attribution"`
- `search --docs "Fluent Bit Kubernetes DaemonSet migration from Fluentd"`
- `search --docs "Parca continuous profiling Go gRPC"`
...
```
search --docs "site:landscape.cncf.io {category or vendor}"
```
