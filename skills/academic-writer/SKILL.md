...
- Citation gap branch: when a claim lacks evidence, weaken or remove rather than fabricate; optionally hand off to `scholar`
- Language branch: non-English target hands off to `translator` after the English pass
...
- If a claim cannot be supported by available evidence → weaken with hedging or remove; if a citation gap is structural, NOTIFY `scholar`.
- If the target language is non-English → finish the English pass, then hand off to `translator`.
...
- The target register is academic English. If the final deliverable is non-English, the user has agreed to a downstream `translator` handoff.
