...
- General web search or non-academic content -> use `search`
- Translating papers -> use `translator`
- PDF parsing only (no sidecar) -> use `pdf`
...
| Validate sidecar | `VALIDATE` | `scholar-lint` |
| Report result | `NOTIFY` | Summary or lint report |

### Tools and instruments
- `scholar-search|resolve|get|lint`
- Knows public API, OpenAlex fallback, sidecar spec, checklist

### Canonical command path
```bash
scholar-search "<query>"
scholar-resolve "<title-or-doi>"
scholar-get "<record-id-or-doi>"
scholar-lint "<paper.knows.yaml>"
```
...
| `PROCESS` | `scholar` skill CLI and lint |
...
13. **Validate before sharing**: run `scholar-lint` after Generate
...
| Mode | Trigger | Output |
|------|---------|--------|
| **Generate** | "create sidecar from this paper / abstract / draft", "generate `.knows.yaml`" | `{paper}.knows.yaml` (host LLM emits, then `scholar-lint` validates) |
...
Use the `scholar` skill subcommands (load via `skill` tool):
...
scholar-resolve "Attention Is All You Need"

# Get by id (knows record_id, OpenAlex W-id, or DOI)
scholar-get "10.48550/arXiv.1706.03762"
```
...
```bash
scholar-search "diffusion super resolution"
scholar-search --year-min 2024 "vision language action"
```
...
scholar-get "knows:generated/reconvla/1.0.0"

# Partial fetch (claims only, ~700 tokens, 93% reduction vs PDF)
scholar-get --section statements "knows:generated/reconvla/1.0.0"
...
scholar-lint --lenient remote.knows.yaml

# Treat warnings as failures (CI mode)
scholar-lint --fail-on-warning paper.knows.yaml
```
