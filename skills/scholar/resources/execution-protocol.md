...
2. If PDF and only path is given -> chain via `pdf` first to extract markdown
...
```bash
scholar-lint {output}.knows.yaml
```
...
1. Locate sidecar: local path or `scholar fetch {record_id}` from knows.academy
...
### Recommended path: `scholar` skill (handles cascade automatically)
...
| Error | Recovery |
|-------|----------|
| `scholar` skill not available | Install the required tooling to enable the CLI |
...
| Source PDF has no text layer | Chain via `pdf` with hybrid OCR mode first |
