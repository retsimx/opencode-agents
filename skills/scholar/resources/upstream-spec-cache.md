...
curl -s https://knows.academy/api/proxy/skill/knows.md \
  > .agents/skills/scholar/resources/upstream-spec-cache.md.new
diff .agents/skills/scholar/resources/upstream-spec-cache.md \
     .agents/skills/scholar/resources/upstream-spec-cache.md.new
```
...
2. **Validation tooling**: upstream references `pip install knows-sidecar`; that package is **not yet on PyPI** (verified 2026-04-25). We ship rule-based validation as the `scholar-lint` CLI subcommand (TypeScript, no Python dependency).
