...
`scholar` queries `knows.academy` first, then falls back to OpenAlex for
...
# OpenAlex by W-id
scholar-get "W2147144213"
```

When `scholar-get` is asked for a `knows:...` id and the platform is
...
- Full-text reading, since neither source provides full text; fetch the OA PDF and use `pdf`
...
```
1. scholar-get <doi>        → get title, authors, year, venue, abstract
2. (optional) download OA PDF   → pdf to extract full text
3. Mode 1 Generate              → host LLM produces .knows.yaml using sidecar-spec.md
4. scholar-lint             → validate locally
```
...
- **knows.academy sidecars are AI-generated**: all have `provenance.origin: machine`. Verified `lint_passed: true` is platform-internal; our local `scholar-lint` finds dangling refs in ~47% (use `--lenient` when consuming)
