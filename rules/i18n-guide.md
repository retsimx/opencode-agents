...
When translating user-facing content, the default voice is `balanced` (content-type defaults — fragments only in label/cell positions). It can be overridden per-task.
...
Workflows that translate user-facing content should respect this setting via the `translator` skill rather than hardcoding a tone.
...
6. **Translation tasks** — for translating UI strings, docs, or marketing copy, use the `/translator` skill
